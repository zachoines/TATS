...
int SACAgent::_save_to_array(torch::nn::Module& from, double* address, int index) {
	...
	for (const auto& val : params) {
		...
		torch::Tensor layer = torch::flatten(val.value().clone().detach());
...
int SACAgent::_load_from_array(torch::nn::Module& to, double* address, int index) {
	...
	for (const auto& val : params) {
		...
		params.find(name)->copy_(torch::from_blob(copy, val.value().data().sizes(), optionsDouble));
	...
	for (const auto& val : buffers) {
		...
		buffers.find(name)->copy_(torch::from_blob(copy, val.value().data().sizes(), optionsDouble));
